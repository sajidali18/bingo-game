...
const BingoGame = () => {
    ...
    const [penalizedNumbers, setPenalizedNumbers] = useState(new Set()); // <-- NEW
    ...
    useEffect(() => {
        ...
        socket.on('boardSelected', (board) => {
            ...
            const freeIndex = 12;
            setMatchedCells(new Set([freeIndex]));
        ...
        socket.on('numberDrawn', (num) => {
            ...
            setPenalizedNumbers(new Set()); // <-- Reset penalty tracker
        });
    ...
    const handleCellClick = (cell, index) => {
        ...
        if (value === currentNumber) {
            ...
            setPenalizedNumbers((prev) => {
                const updated = new Set(prev);
                updated.delete(currentNumber);
                return updated;
            });
        } else {
            if (!penalizedNumbers.has(currentNumber)) {
                setScore((prev) => prev - 10);
                setPenalizedNumbers((prev) => new Set(prev).add(currentNumber));
            }
        }
    };
